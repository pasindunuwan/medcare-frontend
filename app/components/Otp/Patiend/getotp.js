...
export default function Getotp() {
  const { onHandleNext, onHandleBack, FormData, Step } = useFormStatePa();
  ...
  async function handleSubmit(event) {
    ...
    try {
      const response = await fetch("http://localhost:8070/patient/Paverify", {
        method: "POST",
        headers: {
  ...
  async function handleResend(e) {
    ...
    try {
      const response = await fetch(
        "http://localhost:8070/patient/paresendmail",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
