...
export default function Getotp() {
  const { onHandleNext, onHandleBack, FormData, Step } = useFormStatePh();
  ...
  async function handleSubmit(event) {
    ...
    try {
      const response = await fetch(
        "http://localhost:8070/pharmacy/Pharmacyverify",
        {
          method: "POST",
          headers: {
  ...
  async function handleResend(e) {
    ...
    try {
      const response = await fetch(
        "http://localhost:8070/pharmacy/pharmacyresendmail",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
